...
export default function AdminDashboard() {
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'logs' | 'lead_wallets' | 'users'>('overview')
  ...
  return (
    <div className="min-h-screen bg-void-black">
      ...
      <div className="flex">
        {/* Sidebar */}
        <div className={`fixed top-20 left-0 h-[calc(100vh-5rem)] w-64 bg-void-black/90 border-r border-molten-gold/20 z-30 transition-transform duration-300 md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
          }`}>
          <div className="p-6">
            ...
            <nav className="space-y-2">
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-300 ${activeSection === 'overview'
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-300 ${activeSection === 'logs'
                  ? 'bg-molten-gold/20 border border-molten-gold/50 text-molten-gold'
                  : 'bg-void-black/50 border border-molten-gold/10 text-white/60 hover:bg-molten-gold/10 hover:text-white'
                  }`}
              >
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-300 ${activeSection === 'lead_wallets'
                  ? 'bg-molten-gold/20 border border-molten-gold/50 text-molten-gold'
                  : 'bg-void-black/50 border border-molten-gold/10 text-white/60 hover:bg-molten-gold/10 hover:text-white'
                  }`}
              >
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-300 ${activeSection === 'users'
                    ? 'bg-molten-gold/20 border border-molten-gold/50 text-molten-gold'
                    : 'bg-void-black/50 border border-molten-gold/10 text-white/60 hover:bg-molten-gold/10 hover:text-white'
                  }`}
              >
                <div className="flex items-center gap-3">
                  <Users size={18} />
                  <span className="font-orbitron font-semibold">Users</span>
                </div>
                {activeSection === 'users' && <ChevronRight size={16} />}
              </button>
            </nav>
          </div>
        </div>
